...
def build_detection_semisup_train_loader(cfg, mapper=None):
    ...
    logger.info(f"Number of labeled data: {len(dataset_dicts)}")

    if len(dataset_dicts) == 0:
        logger.error("No labeled data available! Please check your dataset.")
    ...
    logger.info(f"Number of labeled data: {len(label_dicts)}")
    logger.info(f"Number of unlabeled data: {len(unlabel_dicts)}")

    if len(label_dicts) == 0:
        logger.error("No labeled data available after the split. Please check your dataset and supervision percentage.")
