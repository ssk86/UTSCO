...
class StandardROIHeadsPseudoLab(StandardROIHeads):
    ...
    def _shared_roi_transform(self, features: Dict[str, torch.Tensor], boxes: List[Boxes]) -> torch.Tensor:
        box_feature = self.box_pooler(features, boxes)
        
        return box_feature        
    # def _shared_roi_transform(self, features: Dict[str, torch.Tensor], boxes: List[torch.Tensor]) -> torch.Tensor:
    #     # 假设 boxes 是一个包含多个 Tensor 的列表
    #     box_features = []
    #     print(features[0].shape)
    #     for box in boxes:
    #         # 直接使用 box，而不是 box.tensor
    #         box_feature = self.box_pooler(features, box)
